...
typedef enum _spi_frame_format { SPI_FF_STANDARD, SPI_FF_DUAL, SPI_FF_QUAD, SPI_FF_OCTAL } spi_frame_format_t;
...
typedef enum _spi_transfer_mode { SPI_TMOD_TRANS_RECV, SPI_TMOD_TRANS, SPI_TMOD_RECV, SPI_TMOD_EEROM } spi_transfer_mode_t;
 ...
 * @return      Void
 */
void spi_init(spi_device_num_t spi_num, spi_work_mode_t work_mode, spi_frame_format_t frame_format, uint64_t data_bit_length, uint32_t endian);
 ...
 *
 */
void spi_init_non_standard(spi_device_num_t spi_num,
 ...
 * @return      Void
 */
// void spi_slave_config(uint8_t int_pin, uint8_t ready_pin, dmac_channel_number_t dmac_channel, uint64_t data_bit_length, uint8_t *data, uint32_t
// len, spi_slave_receive_callback_t callback);
