 ...
 */
#ifndef __DRIVER_GPIOHS_H__
#define __DRIVER_GPIOHS_H__
...
/**
 * @brief      GPIO object
 * The GPIO controller is a peripheral device mapped in the internal memory map, discoverable in the Configuration String. It is responsible for
 * low-level configuration of the actual GPIO pads on the device (direction, pull up-enable, and drive value), as well as selecting between various
 * sources of the controls for these signals. The GPIO controller allows seperate configuration of each of N GPIO bits. Once the interrupt is pending,
 * it will remain set until a 1 is written to the *_ip register at that bit.
 */
