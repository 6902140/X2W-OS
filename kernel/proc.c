...
fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  np->trapframe->s0 = p->trapframe->s0;
  // printf("%d T&F:%d\n",np->trapframe->sp,np->trapframe->sp==p->trapframe->sp);
...
exit(int status)
{
  ...
  for (int i = 0; i < MMAPNUM; i++)
  {
    if (p->mmap_pool[i].used)
      do_munmap(p->mmap_pool[i].start, p->mmap_pool[i].len);
  }
