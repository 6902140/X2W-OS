...
static int 
handle_store_page_fault(pte_t * pte)
{
  // uint64 pa = PTE2PA(*pte);
  // pte |= PTE_W;
  return 0;
...
static int 
handle_page_fault_lazy(uint64 stval)
{
  struct proc *p = myproc();

	uint64 va = PGROUNDDOWN(stval);  // in uvmalloc(), oldsz will round up
	if (uvmalloc(p->pagetable, p->kpagetable, va, va + PGSIZE) == 0) {
  	return -1;
  }
...
int 
handle_page_fault(int kind, uint stval)
{
  printf("page_fault! stval: %p\n", stval);
  struct proc *p = myproc();
  pte_t *pte = walk(p->pagetable, stval, 0);

  if (kind == 1 && pte && (*pte & PTE_RSW1))
  {
    return handle_store_page_fault(pte);
  } else {
    return handle_page_fault_lazy(stval);
  }
  return -1;
}
