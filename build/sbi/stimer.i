...
int64_t stimer_interrupt_handler(strapframe_t *stf_ptr){
    ...
    ({ unsigned long __v = (unsigned long) ((1UL << 0x07)); __asm__ __volatile__( "csrc " "mie" ", %0" : : "rK" (__v) : "memory" ); });
    ({ unsigned long __v = (unsigned long) ((1UL << 0x05)); __asm__ __volatile__( "csrs " "mip" ", %0" : : "rK" (__v) : "memory" ); });
    return 0;
}


void clint_timer_event_start(uint64_t next_ticks){
    write_64_bits((0x2000000 + 0x4000), next_ticks);

    ({ unsigned long __v = (unsigned long) ((1UL << 0x05)); __asm__ __volatile__( "csrc " "mip" ", %0" : : "rK" (__v) : "memory" ); });
    ({ unsigned long __v = (unsigned long) ((1UL << 0x07)); __asm__ __volatile__( "csrs " "mie" ", %0" : : "rK" (__v) : "memory" ); });
}
