...
void pop_off(void)
{
  ...
  if (!intr_get()) {} else {kpanic_spin("include/process.h", 152, __func__, "!intr_get()", "\"pop_off - interruptible\"");};


  if (c->noff>=1) {} else {kpanic_spin("include/process.h", 155, __func__, "c->noff>=1", "\"pop_off\"");};
  c->noff -= 1;
...
void process_statck_map(pagetable_t kernel_pgdir){
    ...
    for(;p<&processes[64];p++){
        ...
        if (pa!=0) {} else {kpanic_spin("kernel/process.c", 31, __func__, "pa!=0", "\"process[%u] stack malloc error\\n\"",p);};
