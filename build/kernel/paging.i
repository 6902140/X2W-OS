...
# 70 "include/kernel/mm.h"
ppool_t *get_ppool(void);
# 80 "include/kernel/mm.h"
vpool_t *get_vpool(void);
# 94 "include/kernel/mm.h"
...
# 187 "include/kernel/paging.h"
void dump_pgd(pgd_t *pgd, addr_t s_vaddr, addr_t e_vaddr);
# 205 "include/kernel/paging.h"
...
static inline offset_t get_vpn(addr_t vaddr, int vpn){
    ...
    if (result != -1) {} else {kpanic_spin("include/kernel/paging.h", 238, __func__, "result != -1", "\"vpn = %d, should be in [2, 1, 0, -1]!\"", vpn);};
    return result;
    __builtin_unreachable();
}
# 250 "include/kernel/paging.h"
static inline addr_t get_ppn(addr_t paddr, int ppn){
    if (ppn == 2)
        ...
        if (False) {} else {kpanic_spin("include/kernel/paging.h", 258, __func__, "False", "\"ppn = %d, should be in [2, 1, 0]!\"");};
    __builtin_unreachable();
}
# 269 "include/kernel/paging.h"
...
# 496 "include/kernel/paging.h"
void create_identical_mapping(void);
# 506 "include/kernel/paging.h"
void create_mmio_mapping(void);
# 517 "include/kernel/paging.h"
void paging_init(void);
# 528 "include/kernel/paging.h"
...
void create_identical_mapping(void){
    ...
    start_addr=(addr_t)_e_kernel,end_addr=start_addr+(16 * 0x100000UL),size=(size_t)(end_addr-start_addr);
    pages = (size + (1 << 12) - 1) / (1 << 12);
    kprintf("\tmapping other RAM, %#X~%#X, %7d Bytes, %2d Pages used (%2d * 4096 = %7d)\n", start_addr, end_addr, size, pages, pages, pages * (1 << 12));
    page_property_t kheap_prot = {(uint64_t) ((1 << 1) | (1 << 2) | (1 << 0) | (1 << 6) | (1 << 7) | (1 << 5))};
    create_mapping((pgd_t *)kernel_pgd, start_addr, start_addr, size, kheap_prot, 0);

    start_addr=(addr_t)((1L << 38) - (1 << 12)),end_addr=start_addr+(1 << 12),size=(size_t)(1 << 12);
    pages = (size + (1 << 12) - 1) / (1 << 12);
    kprintf("\tmapping trampoline, %#X~%#X, %7d Bytes, %2d Pages used (%2d * 4096 = %7d)\n", start_addr, end_addr, size, pages, pages, pages * (1 << 12));
    page_property_t trampoline_prot = {(uint64_t) ((1L << 1) | (1L << 3))};
...
__attribute__((noreturn)) int64_t paging_load_page_fault_exception_handler(ktrapframe_t *ktf_ptr){
    ...
    for (int32_t max = 0x7fffffff; max-- > 0;);
...
void parse_pgd(pgd_t *pgd, addr_t s_vaddr, size_t size){
    ...
    do {
        ...
        if (!is_valid_page(pgd_ent->val) || is_leaf_page(pgd_ent->val))
            ...
            if (False) {} else {kpanic_spin("kernel/paging.c", 272, __func__, "False", "\"Invalid PGD Entry!\"");};
...
void create_mapping(pgd_t *pgd, addr_t vaddr, addr_t paddr, uint64_t size, page_property_t property, uint64_t flags){
    ...
    if (0 < size) {} else {kpanic_spin("kernel/paging.c", 366, __func__, "0 < size", "\"mapping memory size must greater than 0, but size=%d!\"", size);};
...
pt_t* get_level0_pte(pgd_t pageTable, addr_t va, int alloc) {
    ...
    if (va<=(1L << 38)) {} else {kpanic_spin("kernel/paging.c", 400, __func__, "va<=MAX_VA", "\"invalid va\"",0);};
    pt_t* pte;
    for (int level = 2; level > 0; level--) {
        pte = &((pt_t*)pageTable)[(((addr_t)(va) >> (12 + (level)*9)) & 0x1ff)];
        if (*pte & (1L << 0)) {
            pageTable = (pgd_t)((((addr_t)(*pte)) >> 10) << 12);
        } else {
            if (!alloc)
                return ((void *)0);
            pageTable = (pgd_t)alloc_ppage(1);
            if (pageTable == (addr_t)((void *)0))
                return ((void *)0);
            memset((char*)pageTable, 0, (1 << 12));
            *pte = (((addr_t)(pageTable) >> 12) << 10) | (1L << 0);
        }
    }



    return &((pt_t*)pageTable)[(((addr_t)(va) >> (12 + (0)*9)) & 0x1ff)];
}
