...
void create_identical_mapping(void){
    ...
    start_addr=(addr_t)_e_bss,end_addr=(addr_t)0x86400000,size=(size_t) (end_addr - start_addr);
    pages = (size + (1 << 12) - 1) / (1 << 12);
    kprintf("\tmapping total free mem, %#X~%#X, %7d Bytes, %2d Pages used (%2d * 4096 = %7d)\n", start_addr, end_addr, size, pages, pages, pages * (1 << 12));
    page_property_t total_mem_prot = {(uint64_t) ((1 << 1) | (1 << 2) | (1 << 0) | (1 << 6) | (1 << 7) | (1 << 5))};
...
void parse_pgd(pgd_t *pgd, addr_t s_vaddr, size_t size){
    ...
    do {
        ...
        if (!is_valid_page(pgd_ent->val) || is_leaf_page(pgd_ent->val))
            ...
            if (False) {} else {kpanic_spin("kernel/paging.c", 260, __func__, "False", "\"Invalid PGD Entry!\"");};
...
void create_mapping(pgd_t *pgd, addr_t vaddr, addr_t paddr, uint64_t size, page_property_t property, uint64_t flags){
    ...
    if (0 < size) {} else {kpanic_spin("kernel/paging.c", 354, __func__, "0 < size", "\"mapping memory size must greater than 0, but size=%d!\"", size);};
