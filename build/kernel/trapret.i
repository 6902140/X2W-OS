...
void pop_off(void)
{
  ...
  if (!intr_get()) {} else {kpanic_spin("include/process.h", 156, __func__, "!intr_get()", "\"pop_off - interruptible\"");};


  if (c->noff>=1) {} else {kpanic_spin("include/process.h", 159, __func__, "c->noff>=1", "\"pop_off\"");};
  c->noff -= 1;
...
void usertrap(void)
{
    ...
    if ((r_sstatus() & 0x00000100UL) == 0) {} else {kpanic_spin("kernel/trapret.c", 12, __func__, "(r_sstatus() & SSTATUS_SPP) == 0", "\"usertrap: not from user mode %d\\n\"",0);};
