...
void usertrap(void)
{
    ...
    if ((r_sstatus() & 0x00000100UL) == 0) {} else {kpanic_spin("kernel/trapret.c", 15, __func__, "(r_sstatus() & SSTATUS_SPP) == 0", "\"usertrap: not from user mode %d\\n\"",0);};
...
void usertrapret(void)
{
  ...
  uint64_t trampoline_uservec = ((1L << (9 + 9 + 9 + 12 - 1)) - 4096) + (uservec - trampoline);
  ...
  uint64_t satp = ((8L << 60) | (((uint64_t)p->pagetable) >> 12));




  uint64_t trampoline_userret = ((1L << (9 + 9 + 9 + 12 - 1)) - 4096) + (userret - trampoline);
...
void kerneltrap(void)
{
  ...
  if (((sstatus & 0x00000100UL) == 0)) {} else {kpanic_spin("kernel/trapret.c", 119, __func__, "((sstatus & SSTATUS_SPP) == 0)", "\"kerneltrap: not from supervisor mode\\n\"",0);};


   if (intr_get()==0) {} else {kpanic_spin("kernel/trapret.c", 122, __func__, "intr_get()==0", "\"kerneltrap: not from supervisor mode\\n\"",0);};




     kprintf("scause %p\n", scause);
# 138 "kernel/trapret.c"
  w_sepc(sepc);
  w_sstatus(sstatus);
}
