...
addr_t alloc_ppage(Bool kpage){
    ...
    if (bit_idx != -1) {} else {kpanic_spin("kernel/mm.c", 116, __func__, "bit_idx != -1", "\"bit_idx=%d, cannot find a physical page!\"", bit_idx);};
...
addr_t alloc_nppage(size_t cnt,Bool kpage){
    ...
    if (bit_idx != -1) {} else {kpanic_spin("kernel/mm.c", 136, __func__, "bit_idx != -1", "\"bit_idx=%d, cannot find a physical page!\"", bit_idx);};

    spinlock_acquire(&pool->lock);
    for(int i=bit_idx;i-bit_idx<cnt;i++)
        bitmap_set(pool->btmp, i, 1);
    spinlock_release(&pool->lock);


    addr_t ppage = pool->paddr_start + bit_idx * (1 << 12);
    for(int i=bit_idx;i-bit_idx<cnt;i++)
        memset((void *)((char*)ppage+(i-bit_idx)*(1 << 12)), 0, (1 << 12));
...
void free_ppage(addr_t ppage, Bool kpage){
    ...
    if (bit_idx >= 0) {} else {kpanic_spin("kernel/mm.c", 158, __func__, "bit_idx >= 0", "\"bit_idx shouldn't be negative, bit_idx = %d!\"", bit_idx);};
...
void kfree_pages(addr_t pstart,addr_t pend){
    ...
    temp=(char*)(((addr_t)(temp) + (1 << 12) - 1) & ~((1 << 12) - 1));
    for(;temp<(char*)pend;temp+=(1 << 12)){
