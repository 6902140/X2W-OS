...
void free_ppage(addr_t ppage, Bool kpage){
    ...
    if (bit_idx >= 0) {} else {kpanic_spin("kernel/mm.c", 134, __func__, "bit_idx >= 0", "\"bit_idx shouldn't be negative, bit_idx = %d!\"", bit_idx);};
...
addr_t alloc_vpage(vpool_t *vpool, size_t cnt){
    if (vpool != ((void *)0)) {} else {kpanic_spin("kernel/mm.c", 143, __func__, "vpool != NULL", "\"vpool shouldn't be NULL!\"");};

    offset_t bit_idx = bitmap_scan(vpool->btmp, cnt);

    if (bit_idx != -1) {} else {kpanic_spin("kernel/mm.c", 147, __func__, "bit_idx != -1", "\"bit_idx=%d, cannot find a virtual page!\"", bit_idx);};
...
void free_vpage(vpool_t *vpool, addr_t vpage){
    if (vpool != ((void *)0)) {} else {kpanic_spin("kernel/mm.c", 157, __func__, "vpool != NULL", "\"vpool shouldn't be NULL!\"");};

    offset_t bit_idx = (vpage - vpool->vaddr_start) / (1 << 12);
    if (bit_idx >= 0) {} else {kpanic_spin("kernel/mm.c", 160, __func__, "bit_idx >= 0", "\"bit_idx shouldn't be negative, bit_idx = %d!\"", bit_idx);};
    spinlock_acquire(&vpool->lock);
...
void *malloc_page(size_t cnt, Bool kpage){
    if (cnt > 0) {} else {kpanic_spin("kernel/mm.c", 185, __func__, "cnt > 0", "\"cnt must greater than zero, but cut = %d!\"", cnt);};

    if (kpage == True) {} else {kpanic_spin("kernel/mm.c", 187, __func__, "kpage == True", "\"Only kernel page is supported now, you're trying to malloc a user page, kpage=%d!\"", kpage);};
